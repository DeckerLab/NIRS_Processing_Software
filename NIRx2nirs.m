function NIRx2nirs(NIRx_foldername, SD_filename, Nirs_filename)
...
if ~exist('SD_filename','var');
    [file,path] = uigetfile('.SD','Select associated SD file...');
    SD_filename = [path file];  
end
...
if ~exist('Nirs_filename','var');
    [file,path] = uiputfile('*.nirs','Save .nirs file as ...');
    Nirs_filename = [path file];
end

fprintf('   Saving as %s ...\n',Nirs_filename);
save(Nirs_filename,'d','s','t','aux','SD');
